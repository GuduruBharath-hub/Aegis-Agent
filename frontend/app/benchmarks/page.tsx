...
export default function BenchmarksPage() {
    ...
    const latestRunMap = runs.reduce<Record<string, (typeof runs)[0]>>((acc, run) => {
        if (!acc[run.case_id] || new Date(run.run_at) > new Date(acc[run.case_id].run_at)) {
            acc[run.case_id] = run;
        }
        return acc;
    }, {});
                ...
                <div className="grid-3" style={{ marginBottom: '24px' }}>
                    {[
                        ...
                        { label: 'Runs Completed', value: runs.filter((r) => r.status === 'completed').length, icon: <CheckCircle2 size={16} />, color: '#a8e063' },
                        {
                            ...
                            value: `${runs.filter((r) => r.false_verification).length}/${runs.filter((r) => r.status === 'completed').length}`,
                ...
                {view === 'runs' && (
                    <div style={{ background: 'linear-gradient(135deg, rgba(22,22,22,0.97) 0%, rgba(12,12,12,0.99) 100%)', border: '1px solid rgba(212,175,55,0.15)', borderRadius: '12px', overflow: 'hidden' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', padding: '10px 20px', borderBottom: '1px solid rgba(212,175,55,0.08)' }}>
                            {['Scenario', 'Status', 'Expected', 'Actual', 'Run Time'].map((col) => (
                        ...
                        {runs.length === 0 ? (
                            ...
                            runs.map((run) => (
                                <div key={run.id} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', padding: '13px 20px', borderBottom: '1px solid rgba(212,175,55,0.04)', alignItems: 'center' }}>
                                    <span style={{ fontSize: '0.875rem', color: 'rgba(255,215,0,0.85)', fontWeight: 500 }}>{scenarios.find((item) => item.id === run.case_id)?.name ?? run.case_id}</span>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                        {run.status === 'completed' && run.correct ? <CheckCircle2 size={13} color="#a8e063" /> : run.status === 'completed' ? <XCircle size={13} color="#ff6060" /> : <Clock size={13} color="#d4af37" />}
                                        <span style={{ fontSize: '0.8rem', color: run.status === 'completed' && run.correct ? '#a8e063' : run.status === 'completed' ? '#ff6060' : '#ffd700', textTransform: 'capitalize' }}>{run.status}</span>
                                    </div>
